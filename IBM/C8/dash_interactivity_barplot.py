...
def get_graph(entered_year):
    ...
    labels = dict(x='Destination state',y='Number of flights')
    # fig = go.Figure()
    # fig.add_trace(go.Bar(x=bar_data.DestState, y=bar_data.Flights))
    # fig.update_layout(xaxis_title=labels['x'], yaxis_title=labels['y'])
    fig = px.bar(data_frame=bar_data, x='DestState', y='Flights',
                  labels={'DestState':labels['x'], 'Flights': labels['y']})
